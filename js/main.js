...
const sectionData = sectionJson;
const socialData = socialJson;
...
function updateActiveNav() {
    ...
    document
        .querySelectorAll(".side-nav-link, .mobile-nav-link, .header-link")
        .forEach((link) => {
    ...
    if (servicesEl && data.services) {
        ...
        <div class="service-card service-card--${service.color} animate-scale-in animate-delay-${i + 1}">
          ...
          <ul class="service-card__features">
            ${service.features.map((f) => `<li><i class="ri-check-line"></i>${f}</li>`).join("")}
          </ul>
        </div>
      `)
            .join("");
...
function renderCompany() {
    ...
    function renderCompanyProjects(filter = "all") {
        if (!gridEl)
            return;
        gridEl.innerHTML = "";
...
// Export 
export { renderAbout, renderPortfolio, renderCompany, renderConsulting, renderLinks, renderContact, openModal, closeModal };
