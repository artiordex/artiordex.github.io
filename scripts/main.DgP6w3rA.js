(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const s of n.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function i(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function o(a){if(a.ep)return;a.ep=!0;const n=i(a);fetch(a.href,n)}})();const U=[{id:"about",label:"자기소개",icon:"ri-user-line"},{id:"portfolio",label:"포트폴리오",icon:"ri-briefcase-line"},{id:"consulting",label:"컨설팅",icon:"ri-lightbulb-line"},{id:"company",label:"회사소개",icon:"ri-building-line"},{id:"links",label:"링크",icon:"ri-links-line"},{id:"contact",label:"연락처",icon:"ri-mail-line"}],x={sections:U},X=[{code:"ko",label:"KR",name:"한국어"},{code:"en",label:"EN",name:"English"},{code:"ja",label:"JP",name:"日本語"},{code:"vi",label:"VN",name:"Tiếng Việt"},{code:"de",label:"DE",name:"Deutsch"},{code:"es",label:"ES",name:"Español"},{code:"zh",label:"ZH",name:"中文"}],j={default:"ko",languages:X},Z={mountId:"layout-header",navListId:"header-nav-list",activeClass:"active",linkClass:"header-link",spyThreshold:.5,hideOnScrollDown:!0,scrollThreshold:100};class ee{config;sections=[];navListEl=null;headerEl=null;links=[];observer=null;lastScrollY=0;constructor(t={}){this.config={...Z,...t}}init(){this.renderSkeleton(),this.loadSections(),this.sections.length!==0&&(this.renderNav(),this.setupScrollSpy(),this.bindClickEvents(),this.bindLanguageSelector(),this.config.hideOnScrollDown&&this.setupScrollHide())}destroy(){this.observer?.disconnect(),window.removeEventListener("scroll",this.updateHeaderVisibility)}renderSkeleton(){const t=document.getElementById(this.config.mountId);if(!t){console.error(`[Header] Mount element not found: #${this.config.mountId}`);return}t.innerHTML=`
  ...
  `}function fe(e,t){switch(e.type){case"profile_strengths":return ye(e,t);case"timeline_portrait":return _e(e);case"core_competency":return ke(e);case"education_certifications":return $e(e);case"persona":return we(e);case"summary":return Ae(e);default:return'<div style="grid-column:1/-1;">Unsupported slide type</div>'}}function ye(e,t){const i=e.profile??{},o=Array.isArray(e.keywords)?e.keywords:[],a=Array.isArray(e.strengths)?e.strengths:[],n=Array.isArray(i.description)?i.description:typeof i.description=="string"?[i.description]:[],s=["hero-tag--red","hero-tag--orange","hero-tag--yellow","hero-tag--green","hero-tag--blue","hero-tag--purple"],m=t.map(u=>{const r=String(u.url??""),c=/^https?:\/\//i.test(r)?'target="_blank" rel="noopener noreferrer"':'rel="noopener noreferrer"';return`
        <a href="${h(r)}"
          ${c}
          aria-label="${h(u.platform??"")}"
          title="${h(u.platform??"")}">
          <i class="${h(u.icon??"")}"></i>
        </a>
      `}).join("");return`
    ...
    <div id="profile-section">
      ...
      <h1 class="hero-title">${d(i.name??"")}</h1>
      <p class="hero-subtitle">${d(i.title??"")}</p>

      <div class="hero-description">
        ${n.map(u=>`<p>${d(u)}</p>`).join("")}
      </div>

      <div class="hero-tags" id="profile-highlights">
        ${o.map(u=>`<span class="hero-tag ${s[Math.floor(Math.random()*s.length)]}">${d(u)}</span>`).join("")}
      </div>
      ...
      <div class="strengths-list">
        ${a.map(u=>`
            <div class="strength-item" style="margin-bottom:1.25rem;">
              <div style="font-weight:700; margin-bottom:0.5rem;">${d(u.title??"")}</div>
              <div style="color:inherit; line-height:1.7;">
                ${(Array.isArray(u.description)?u.description:[]).map(r=>`<div>• ${d(r)}</div>`).join("")}
              </div>
            </div>
          `).join("")}
      </div>
  ...
  `}function _e(e){const t=e.title??"주요 연혁",i=e.portraitImage??"",o=Array.isArray(e.timeline)?e.timeline:[];return`
    <div class="hero-skills-card">
      <h3 class="hero-skills-card__title">${d(t)}</h3>
      <div class="timeline-accordion">
        ${o.map((a,n)=>`
            <div class="accordion-item ${n===0?"active":""}">
              <button class="accordion-header" type="button" aria-expanded="${n===0?"true":"false"}">
                <span class="accordion-year">${d(a.year??"")}</span>
                <span class="accordion-title">${d(a.title??"")}</span>
                <i class="ri-arrow-down-s-line accordion-icon"></i>
              </button>
              <div class="accordion-content" ${n===0?'style="max-height: 500px;"':""}>
                <div class="accordion-body">
                  ${(Array.isArray(a.description)?a.description:[]).map(s=>`<div>${d(s)}</div>`).join("")}
                </div>
              </div>
            </div>
          `).join("")}
  ...
  `}function ke(e){const t=e.title??"자기소개서 핵심역량",i=Array.isArray(e.competencies)?e.competencies:[],o=Array.isArray(e.summaryKeywords)?e.summaryKeywords:[],a=e.brandingQuote??null;return`
    <div class="hero-skills-card" style="grid-column:1/-1;">
      ...
      <div class="competency-tabs">
        ${i.map((n,s)=>`
            <button 
              class="competency-tab ${s===0?"active":""}" 
              type="button"
              data-tab="${s}"
            >
              ${d(n.name??"")}
      ...
      <div class="competency-tab-contents">
        ${i.map((n,s)=>`
            <div class="competency-tab-content ${s===0?"active":""}" data-tab-content="${s}">
              <div class="competency-summary">${d(n.summary??"")}</div>
              <div class="competency-evidence">
                ${(Array.isArray(n.evidence)?n.evidence:[]).map(m=>`<div>• ${d(m)}</div>`).join("")}
              </div>
              <div class="competency-result">${d(n.result??"")}</div>
            </div>
          `).join("")}
      </div>
      ...
      ${o.length?`
        <div class="competency-keywords">
          ...
          <div class="competency-keywords__list">
            ${o.map(n=>`<span class="hero-tag hero-tag--primary">${d(n)}</span>`).join("")}
          </div>
        </div>
      `:""}
      ...
      ${a?`
        <div class="competency-quote">
          <span class="competency-quote__line">${d(Array.isArray(a.lines)?a.lines.join(" "):a.lines??"")}</span>
          <span class="competency-quote__highlight">${d(a.highlight??"")}</span>
        </div>
      `:""}
    </div>
  `}function $e(e){const t=Array.isArray(e.education)?e.education:[],i=Array.isArray(e.certifications)?e.certifications:[],o=r=>{if(!r||r==="—"||r==="-")return"—";const l=r.match(/(\d{4})\s*$/);if(l)return l[1];const c=r.match(/^(\d{4})/);return c?c[1]:"—"},a=t.reduce((r,l)=>{const c=String(l.year??o(l.period));return r[c]||(r[c]=[]),r[c].push(l),r},{}),n=i.reduce((r,l)=>{const c=String(l.year??"—");return r[c]||(r[c]=[]),r[c].push(l),r},{}),s=r=>r.sort((l,c)=>{const v=parseInt(l,10),$=parseInt(c,10);return isNaN(v)&&isNaN($)?0:isNaN(v)?1:isNaN($)?-1:$-v}),m=s(Object.keys(a)),u=s(Object.keys(n));return`
    <div class="hero-skills-card">
      <h3 class="hero-skills-card__title">학력</h3>
      <div class="edu-cert-timeline">
        ${m.map(r=>`
            <div class="edu-cert-year-group">
              <div class="edu-cert-year">${d(r)}</div>
              <div class="edu-cert-items">
                ${a[r].map(l=>`
                    <div class="edu-cert-item">
                      <div class="edu-cert-item__title">${d(l.degree??"")}</div>
                      <div class="edu-cert-item__sub">${d(l.school??"")}</div>
                    </div>
                  `).join("")}
              </div>
            </div>
              ...
              <div class="edu-cert-items">
                ${n[r].map(l=>`
                    <div class="edu-cert-item">
                      ...
                      <div>
                        <div class="edu-cert-item__title">${d(l.name??"")}</div>
                        <div class="edu-cert-item__sub">${d(l.issuer??"")}</div>
                      </div>
                    </div>
                  `).join("")}
              </div>
  ...
  `}function we(e){const t=e.header??{},i=Array.isArray(e.badges)?e.badges:[],o=Array.isArray(e.table)?e.table:[],a=Array.isArray(e.cards)?e.cards:[];return`
    <div class="hero-skills-card persona-card" style="grid-column:1/-1;">
      <!-- 헤더 섹션 -->
      <div class="persona-header">
        <div class="persona-header__info">
          <h3 class="persona-header__name">${d(t.name??"")}</h3>
          <p class="persona-header__bio">${d(t.bio??"")}</p>
          <div class="persona-badges">
            ${i.map(n=>`<span class="persona-badge">${d(n.label??"")}</span>`).join("")}
          </div>
        </div>
      </div>
      ...
      <div class="persona-table-wrapper">
        <table class="persona-table">
          ${o.map((n,s)=>`
              <tr>
                <td class="persona-table__key">${d(n.k??"")}</td>
                <td class="persona-table__value">${d(n.v??"")}</td>
              </tr>
            `).join("")}
        </table>
      </div>
          ...
          ${a.map(n=>`
              <div class="persona-mini-card">
                ...
                <ul class="persona-mini-card__list">
                  ${(Array.isArray(n.items)?n.items:[]).map(s=>`<li>${d(s)}</li>`).join("")}
                </ul>
              </div>
            `).join("")}
        </div>
  ...
  `}function Ae(e){const t=Array.isArray(e.sections)?e.sections:[];return`
    <div class="hero-skills-card" style="grid-column:1/-1;">
      <h3 class="hero-skills-card__title">${d(e.title??"자기소개서 요약")}</h3>

      <div style="display:flex; flex-direction:column; gap:1.25rem;">
        ${t.map(i=>`
            <div>
              <div style="font-weight:900; margin-bottom:0.35rem;">${d(i.title??"")}</div>
              <div style="line-height:1.8; opacity:0.9;">${d(i.text??"")}</div>
            </div>
          `).join("")}
      </div>
    </div>
  `}function Le(){const e=document.getElementById("about-carousel");if(!e)return;const t=e.querySelector("#carousel-container"),i=Array.from(e.querySelectorAll(".carousel-slide")),o=Array.from(e.querySelectorAll(".indicator")),a=e.querySelector("#prev-btn"),n=e.querySelector("#next-btn");if(!t||i.length===0||!a||!n)return;let s=0;const m=()=>{t.style.transform=`translateX(${-s*100}%)`,o.forEach((c,v)=>c.classList.toggle("active",v===s))},u=c=>{s=Math.max(0,Math.min(c,i.length-1)),m()},r=()=>u((s+1)%i.length),l=()=>u((s-1+i.length)%i.length);n.addEventListener("click",r),a.addEventListener("click",l),o.forEach((c,v)=>{c.addEventListener("click",()=>u(v))}),document.addEventListener("keydown",c=>{c.key==="ArrowLeft"&&l(),c.key==="ArrowRight"&&r()}),m()}function Ie(){const e=document.querySelectorAll(".accordion-item");e.forEach(t=>{const i=t.querySelector(".accordion-header"),o=t.querySelector(".accordion-content");!i||!o||i.addEventListener("click",()=>{const a=t.classList.contains("active");e.forEach(n=>{n.classList.remove("active");const s=n.querySelector(".accordion-content"),m=n.querySelector(".accordion-header");s&&(s.style.maxHeight="0"),m&&m.setAttribute("aria-expanded","false")}),a||(t.classList.add("active"),o.style.maxHeight=o.scrollHeight+"px",i.setAttribute("aria-expanded","true"))})})}function Se(){const e=document.querySelectorAll(".competency-tab"),t=document.querySelectorAll(".competency-tab-content");e.forEach(i=>{i.addEventListener("click",()=>{const o=i.dataset.tab;e.forEach(n=>n.classList.remove("active")),t.forEach(n=>n.classList.remove("active")),i.classList.add("active");const a=document.querySelector(`.competency-tab-content[data-tab-content="${o}"]`);a&&a.classList.add("active")})})}function d(e){return String(e??"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function h(e){return d(e).replaceAll(`
  ...
  `:""}function Be(e,t){const i=document.getElementById("portfolio-filter-buttons");i&&(i.innerHTML="",e.forEach(o=>{const a=document.createElement("button");a.type="button",a.className="filter-btn",a.textContent=o.label,a.dataset.filter=o.id,o.id==="all"&&a.classList.add("active"),a.addEventListener("click",()=>{i.querySelectorAll(".filter-btn").forEach(n=>n.classList.remove("active")),a.classList.add("active"),F(o.id,t)}),i.appendChild(a)}))}function F(e,t){const i=document.getElementById("portfolio-grid");if(!i)return;const o=["primary","secondary","purple","green"];i.innerHTML="",(e==="all"?t:t.filter(n=>n.category===e)).forEach((n,s)=>{const m=document.createElement("article");m.className=`portfolio-item animate-scale-in animate-delay-${s%6+1}`,m.innerHTML=`
      ...
      <div class="portfolio-item__body">
        <div class="portfolio-item__header">
          <span class="portfolio-item__category">${He(e,n.category)}</span>
        ...
        <div class="portfolio-item__tags">
          ${(n.tags||[]).map((u,r)=>`<span class="portfolio-item__tag portfolio-item__tag--${o[r%o.length]}">${u}</span>`).join("")}
        </div>

        ${n.badges?Ne(n.badges):""}

        <button type="button" class="portfolio-item__link">
          자세히 보기 →
        </button>
      </div>
    `,m.querySelector(".portfolio-item__link")?.addEventListener("click",()=>Ve(n)),i.appendChild(m)}),b()}function Ne(e){return`
    ...
    <section class="section">
      <div class="container">
        ...
        <div class="problems-grid">
          ${o.map((l,c)=>`
              <div class="problem-card animate-scale-in" style="animation-delay:${c*.15}s">
    ...
    <section class="section">
      <div class="container">
        ...
        <div class="consulting-grid">
          ${a.map((l,c)=>`
              <div class="consulting-card animate-scale-in" style="animation-delay:${c*.2}s">
                ...
                <ul class="consulting-card__list">
                  ${(l.deliverables??[]).map(v=>`<li><i class="ri-check-line"></i>${v}</li>`).join("")}
                </ul>
              </div>
            `).join("")}
        </div>
          ...
          <div class="process-grid">
            ${n.map((l,c)=>`
                <div class="process-step animate-slide-up">
                  <div class="process-step__icon process-step__icon--${c+1}">
                    <i class="ri-number-${c+1}"></i>
                  </div>
...
구조를 만듭니다.`,subtitle:"아티올덱스와 함께 지속 가능한 디지털 전환을 시작하세요. 단순한 개발 파트너가 아닌, 성장하는 비즈니스 파트너가 되겠습니다.",email:"contact@artiordex.com"},ot={hero:Xe,services:Ze,solutions:et,timeline:tt,vision:it,closing:at},nt=ot;function st(){const e=document.getElementById("company");if(!e)return;e.className="";const{hero:t,services:i,solutions:o,timeline:a,vision:n,closing:s}=nt;e.innerHTML=`
  ...
  `:""}function ct(e,t){const i=o=>typeof o=="object"&&o.name;return`
...
`).map(i=>`<span>${i}</span>`).join("<br>")}
  ...
  `:""}function wt(e,t){return`
    <div class="notebook-page ${t%2===0?"notebook-page--left":"notebook-page--right"}">
      ...
      <div class="notebook-page__header">
        <div class="notebook-page__icon" style="background: ${It(t)}">
  ...
  `:""}function It(e){const t=["linear-gradient(135deg, #667eea 0%, #764ba2 100%)","linear-gradient(135deg, #11998e 0%, #38ef7d 100%)"];return t[e%t.length]}const St={title:"함께 이야기해요",subtitle:"프로젝트 문의, 협업 제안, 컨설팅 요청 등 어떤 내용이든 편하게 연락해주세요",icon:"ri-chat-smile-3-line"},Et={email:{label:"이메일",value:"artiordex@gmail.com",icon:"ri-mail-line"},phone:{label:"전화번호",value:"+82-10-2589-3152",icon:"ri-phone-line"},location:{label:"위치",value:"Gwangmyeong-si, Gyeonggi-do, South Korea",icon:"ri-map-pin-line"},hours:{label:"업무 시간",value:"월-토 · 09:00 - 22:00",icon:"ri-time-line"}},jt=[{platform:"Slack",url:"https://slack.com/artiordex",icon:"ri-slack-fill"},{platform:"KaKaoTalk",url:"https://kakao.com/artiordex",icon:"ri-kakao-talk-fill"},{platform:"Discord",url:"https://discord.com/artiordex",icon:"ri-discord-line"}],Tt={title:"메시지 보내기",description:"아래 양식을 작성해주시면 빠르게 회신드리겠습니다.",fields:[{id:"name",label:"이름",placeholder:"이름을 입력하세요",type:"text",required:!0},{id:"email",label:"이메일",placeholder:"메일 주소를 입력하세요",type:"email",required:!0},{id:"subject",label:"제목",placeholder:"문의 제목을 입력하세요",type:"text",required:!0},{id:"message",label:"내용",placeholder:"문의 내용을 상세히 작성해주세요",type:"textarea",required:!0}],submit:{label:"문의 보내기",icon:"ri-send-plane-2-line"}},qt={hero:St,contactInfo:Et,socialLinks:jt,form:Tt};class f{constructor(t=0,i="Network Error"){this.status=t,this.text=i}}const Pt=()=>{if(!(typeof localStorage>"u"))return{get:e=>Promise.resolve(localStorage.getItem(e)),set:(e,t)=>Promise.resolve(localStorage.setItem(e,t)),remove:e=>Promise.resolve(localStorage.removeItem(e))}},p={origin:"https://api.emailjs.com",blockHeadless:!1,storageProvider:Pt()},E=e=>e?typeof e=="string"?{publicKey:e}:e.toString()==="[object Object]"?e:{}:{},Ct=(e,t="https://api.emailjs.com")=>{if(!e)return;const i=E(e);p.publicKey=i.publicKey,p.blockHeadless=i.blockHeadless,p.storageProvider=i.storageProvider,p.blockList=i.blockList,p.limitRate=i.limitRate,p.origin=i.origin||t},O=async(e,t,i={})=>{const o=await fetch(p.origin+e,{method:"POST",headers:i,body:t}),a=await o.text(),n=new f(o.status,a);if(o.ok)return n;throw n},V=(e,t,i)=>{if(!e||typeof e!="string")throw"The public key is required. Visit https://dashboard.emailjs.com/admin/account";if(!t||typeof t!="string")throw"The service ID is required. Visit https://dashboard.emailjs.com/admin";if(!i||typeof i!="string")throw"The template ID is required. Visit https://dashboard.emailjs.com/admin/templates"},xt=e=>{if(e&&e.toString()!=="[object Object]")throw"The template params have to be the object. Visit https://www.emailjs.com/docs/sdk/send/"},K=e=>e.webdriver||!e.languages||e.languages.length===0,G=()=>new f(451,"Unavailable For Headless Browser"),Dt=(e,t)=>{if(!Array.isArray(e))throw"The BlockList list has to be an array";if(typeof t!="string")throw"The BlockList watchVariable has to be a string"},Mt=e=>!e.list?.length||!e.watchVariable,Rt=(e,t)=>e instanceof FormData?e.get(t):e[t],W=(e,t)=>{if(Mt(e))return!1;Dt(e.list,e.watchVariable);const i=Rt(t,e.watchVariable);return typeof i!="string"?!1:e.list.includes(i)},Q=()=>new f(403,"Forbidden"),Bt=(e,t)=>{if(typeof e!="number"||e<0)throw"The LimitRate throttle has to be a positive number";if(t&&typeof t!="string")throw"The LimitRate ID has to be a non-empty string"},Nt=async(e,t,i)=>{const o=Number(await i.get(e)||0);return t-Date.now()+o},J=async(e,t,i)=>{if(!t.throttle||!i)return!1;Bt(t.throttle,t.id);const o=t.id||e;return await Nt(o,t.throttle,i)>0?!0:(await i.set(o,Date.now().toString()),!1)},Y=()=>new f(429,"Too Many Requests"),Ht=async(e,t,i,o)=>{const a=E(o),n=a.publicKey||p.publicKey,s=a.blockHeadless||p.blockHeadless,m=a.storageProvider||p.storageProvider,u={...p.blockList,...a.blockList},r={...p.limitRate,...a.limitRate};return s&&K(navigator)?Promise.reject(G()):(V(n,e,t),xt(i),i&&W(u,i)?Promise.reject(Q()):await J(location.pathname,r,m)?Promise.reject(Y()):O("/api/v1.0/email/send",JSON.stringify({lib_version:"4.4.1",user_id:n,service_id:e,template_id:t,template_params:i}),{"Content-type":"application/json"}))},Ft=e=>{if(!e||e.nodeName!=="FORM")throw"The 3rd parameter is expected to be the HTML form element or the style selector of the form"},Ot=e=>typeof e=="string"?document.querySelector(e):e,Vt=async(e,t,i,o)=>{const a=E(o),n=a.publicKey||p.publicKey,s=a.blockHeadless||p.blockHeadless,m=p.storageProvider||a.storageProvider,u={...p.blockList,...a.blockList},r={...p.limitRate,...a.limitRate};if(s&&K(navigator))return Promise.reject(G());const l=Ot(i);V(n,e,t),Ft(l);const c=new FormData(l);return W(u,c)?Promise.reject(Q()):await J(location.pathname,r,m)?Promise.reject(Y()):(c.append("lib_version","4.4.1"),c.append("service_id",e),c.append("template_id",t),c.append("user_id",n),O("/api/v1.0/email/send-form",c))},z={init:Ct,send:Ht,sendForm:Vt,EmailJSResponseStatus:f},Kt=void 0,Gt=void 0,Wt=void 0;z.init(Kt);const Qt=qt;function Jt(){const e=document.getElementById("contact");if(!e)return;e.className="";const{hero:t,intro:i,contactInfo:o,socialLinks:a,form:n}=Qt,s=t||i,m={email:"primary",phone:"secondary",location:"purple",hours:"green"},u=Object.entries(o).map(([r,l])=>({...l,color:m[r]}));e.innerHTML=`
    <!-- CTA 히어로 -->
    ${s?Yt(s):""}
              ...
              <div class="contact-info-list">
                ${u.map(r=>`
                  <div class="contact-info-item">
                    <div class="contact-info-item__icon contact-info-item__icon--${r.color}">
                      <i class="${r.icon}"></i>
                    </div>
                    <div class="contact-info-item__content">
                      <span class="contact-info-item__label">${r.label}</span>
                      <span class="contact-info-item__value">${r.value}</span>
                    </div>
                  </div>
                `).join("")}
              </div>
                  ...
                  <div class="contact-social__links">
                    ${a.map(r=>`
                      <a href="${r.url}" target="_blank" rel="noopener noreferrer" 
                         class="contact-social__link" title="${r.platform}">
                        <i class="${r.icon}"></i>
                      </a>
                    `).join("")}
                  </div>
  ...
  `:""}function zt(e){const t=e.type==="textarea";return`
  ...
  `}function Ut(){const e=document.getElementById("contact-form");if(!e)return;const t=Array.from(e.querySelectorAll("input, textarea"));function i(o){const a=o.id,n=o.value.trim(),s=document.getElementById(`${a}-error`);return s&&(s.textContent="",s.classList.remove("show")),o.required&&!n?(s&&(s.textContent="필수 입력 항목입니다.",s.classList.add("show")),!1):a==="email"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n)?(s&&(s.textContent="올바른 이메일 형식을 입력해주세요.",s.classList.add("show")),!1):!0}t.forEach(o=>{o.addEventListener("blur",()=>i(o))}),e.addEventListener("submit",async o=>{if(o.preventDefault(),!t.every(i))return;const a=e.querySelector(".contact-form__submit"),n=a.innerHTML;a.disabled=!0,a.innerHTML='<i class="ri-loader-4-line spin"></i> 전송 중...';const s={name:document.getElementById("name").value,email:document.getElementById("email").value,subject:document.getElementById("subject")?.value||"",message:document.getElementById("message").value,time:Xt()};try{await z.send(Gt,Wt,s),e.reset(),a.innerHTML='<i class="ri-check-line"></i> 전송 완료!',a.classList.add("success"),setTimeout(()=>{a.disabled=!1,a.innerHTML=n,a.classList.remove("success")},3e3)}catch(m){console.error("메일 전송 실패",m),a.innerHTML='<i class="ri-error-warning-line"></i> 전송 실패',a.classList.add("error"),setTimeout(()=>{a.disabled=!1,a.innerHTML=n,a.classList.remove("error")},3e3)}})}function Xt(){return new Date().toLocaleString("ko-KR",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit"})}const Zt=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&t.target.classList.add("visible","revealed")})},{threshold:.15});function ei(){document.querySelectorAll(".animate-fade-in, .animate-slide-up, .animate-scale-in, .scroll-reveal").forEach(e=>Zt.observe(e))}document.addEventListener("DOMContentLoaded",()=>{te(),oe(),ce(),ve(),De(),st(),Ue(),kt(),Jt(),ei(),L(),de()});
